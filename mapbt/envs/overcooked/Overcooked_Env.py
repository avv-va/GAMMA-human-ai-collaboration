...
from mapbt.envs.overcooked.script_agent import SCRIPT_AGENTS
...
class OvercookedEnv(object):
    ...
    def reset(self, regen_mdp=True, outside_info={}):
        ...
        if self.start_state_fn is None:
            ...
            if p <= self.start_state_fn:
                self.state = self.mdp.get_random_start_state_fn(random_start_pos=True)()
            else:
                self.state = self.mdp.get_standard_start_state()
        else:
        ...
        rewards_dict = {
            ...
            "cumulative_category_rewards_by_agent": 0,
        }
    ...
    def _update_game_stats(self, infos):
        ...
        self.game_stats["cumulative_category_rewards_by_agent"] = self.game_stats["cumulative_category_rewards_by_agent"] + self.vectorize_shaped_info(infos["shaped_info_by_agent"])
...
class Overcooked:
    ...
    def __init__(self, all_args, run_dir, baselines_reproducible=False, featurize_type=("ppo", "ppo"), stuck_time=4, rank=None):
        """
        ...
        self.base_mdp = OvercookedGridworld.from_layout_name(all_args.layout_name, old_dynamics=all_args.old_dynamics, rew_shaping_params=rew_shaping_params)
        self.base_env = OvercookedEnv.from_mdp(self.base_mdp, horizon=self.episode_length, start_state_fn=self.random_start_prob)
    ...
    def reset_featurize_type(self, featurize_type=("ppo", "ppo")):
        ...
        for i in range(2):
            ...
            self.action_space.append(Discrete(len(Action.ALL_ACTIONS)))
    ...
    def _setup_observation_space(self):
        ...
        self.ppo_observation_space = Box(np.float32(low), np.float32(high), dtype=np.float32)
        ...
        self.bc_observation_space = Box(np.float32(low), np.float32(high), dtype=np.float32)
    ...
    def _setup_share_observation_space(self):
        ...
        return Box(np.float32(low), np.float32(high), dtype=np.float32)
    ...
    def step(self, action):
        ...
        if self.use_phi:
            ...
            if self.use_hsp:
                ...
                assert len(self.w0) == len(self.w1) == 21 + 1
        ...
        info["stepwise_shaped_info_by_agent"] = info["shaped_info_by_agent"]
    ...
    def render(self):
        try:
            ...
            imageio.mimsave(save_dir + f'/reward_{self.traj["ep_returns"][0]}.gif', imgs, fps=10)
            imgs_dir = os.listdir(save_dir)
            for img_path in imgs_dir:
                img_path = save_dir + '/' + img_path
                if 'png' in img_path:
